...
def safedelete_manager_factory(manager_superclass, queryset_superclass, visibility=DELETED_INVISIBLE):
    ...
    visibility_name = {
        DELETED_INVISIBLE: 'DELETED_INVISIBLE',
        DELETED_VISIBLE_BY_PK: 'DELETED_VISIBLE_BY_PK'
    }.get(visibility)

    # this still will fail if multiple copies of the same params are passed;
    # it's just generally a bad pattern
    sdqs_classname = '{0}SafeDeleteQueryset__{1}'.format(
        queryset_superclass.__name__, visibility_name)
    ...
    sdm_classname = '{0}SafeDeleteManager__{1}'.format(
        manager_superclass.__name__, visibility_name)
